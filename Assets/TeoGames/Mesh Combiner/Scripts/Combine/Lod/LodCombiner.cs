...
namespace TeoGames.Mesh_Combiner.Scripts.Combine.Lod {
	...
	public class LodCombiner : MonoBehaviour, IChunkContainer<LodCombinable, LodUpdateQueueItem>, IAsyncCombiner {
		...
		public Task UpdateTask => updateQueue.UpdateTask.ContinueWith(() => chunk.UpdateTask);
		...
		public bool IsBaked(LodCombinable combinable) {
			if (Instances.TryGetValue(combinable, out var item)) return item.container.IsBaked(combinable, item.lodKey);
