        ...
        where T : unmanaged
    {
        ...
        static int BuildHashCode(in DynamicArrayRef<T> callstackRef, long depth, bool inverted)
        {
            ...
            unchecked
            {
                ...
                if (inverted)
                {
                    hashCode = MixHash(hashCode, callstackRef[callstackRef.Count - 1].GetHashCode());
                    hashCode = MixHash(hashCode, depth.GetHashCode());
                    for (long i = 1; i < depth; ++i)
                    {
                        hashCode = MixHash(hashCode, callstackRef[callstackRef.Count - 1 - i].GetHashCode());
                    }
                }
                else
                {
                    hashCode = MixHash(hashCode, callstackRef[0].GetHashCode());
                    hashCode = MixHash(hashCode, depth.GetHashCode());
                    for (long i = 1; i < depth; ++i)
                    {
                        hashCode = MixHash(hashCode, callstackRef[i].GetHashCode());
                    }
                }

                return FinalizeHash(hashCode);
            }
        }

        [MethodImpl(MethodImplementationHelper.AggressiveInlining)]
        static uint MixHash(uint hashCode, int value)
        {
            hashCode ^= (uint)value;
            hashCode *= 16777619u;
        ...
        [MethodImpl(MethodImplementationHelper.AggressiveInlining)]
        static int FinalizeHash(uint hashCode)
        {
