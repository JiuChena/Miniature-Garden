...
namespace TeoGames.Mesh_Combiner.Scripts.Combine {
	...
	[HelpURL("https://teogames.gitbook.io/dynamic-mesh-combiner/components/mc-combinable")]
	public partial class Combinable : AbstractCombinable {
		...
		protected AbstractMeshCombiner LastCombiner;
		...
		protected virtual void OnTransformParentChanged() {
			...
			if (cur == LastCombiner) return;

			if (LastCombiner) LastCombiner.Exclude(this);
		...
		public override AbstractMeshCombiner GetCombiner() {
			if (!LastCombiner) LastCombiner = GetCurrentCombiner();
